...
public class Transaction {
    ...
    public static Transaction newCoinbaseTX(String to, String data) {
        ...
        // 创建交易输入，(输入其实是不存在的，pubKey字段随意发挥)
        TXInput txInput = new TXInput(new byte[]{}, -1, null, data.getBytes());
        // 创建交易输出(VALUE=SUBSIDY,pubKeyHash=to)
    ...
    public boolean isCoinbase() {
        return this.getInputs().length == 1//输入只有一个
                && this.getInputs()[0].getTxId().length == 0//没有输入交易的id
                && this.getInputs()[0].getTxOutputIndex() == -1;//输入id的输出索引为-1
    }
    ...
    public void sign(BCECPrivateKey privateKey, Map<String, Transaction> prevTxMap) throws Exception {
        ...
        // 再次验证一下交易信息中的交易输入是否正确，也就是能否查找对应的交易数据(去UTXO里面找)
        ...
        for (int i = 0; i < txCopy.getInputs().length; i++) {
            ...
            // 获取交易输入所对应的上一笔交易中的交易输出,得到多组输出中的某一组特指的
    ...
    public boolean verify(Map<String, Transaction> prevTxMap) throws Exception {
        ...
        Map<String , byte[]> utxo = RocksDBUtils.getInstance().getChainstateBucket();
        Integer inputcount =0;
        Integer outputcount =0;
        for (TXOutput txOutput : this.getOutputs()) {
            outputcount+=txOutput.getValue();
        }
        // 再次验证一下交易信息中的交易输入是否正确，也就是能否查找对应的交易数据
        // 被花费的UTXO的总金额必须大于等于该交易产生的UTXO总金额。
        for (TXInput txInput : this.getInputs()) {
            if (prevTxMap.get(Hex.encodeHexString(txInput.getTxId())) == null) {
                throw new RuntimeException("ERROR: Previous transaction is not correct");
            }
            //String 是 txid   ，  byte 是 txoutput[]
            //根据输入中的txid找到对应的 utxos
            byte[] bytes = utxo.get(Hex.encodeHexString(txInput.getTxId()));
            TXOutput[] txOutputs =(TXOutput[])SerializeUtils.deserialize(bytes);
            //从之前的交易找得到所引用的具体的 txoutput
            Transaction extx = prevTxMap.get(Hex.encodeHexString(txInput.getTxId()));
            TXOutput txOutput=extx.getOutputs()[txInput.getTxOutputIndex()];
            //在 utxos 中 找到 对应的 utxo
            for(TXOutput temp : txOutputs){
                if(byteEqual(txOutput.getPubKeyHash(),temp.getPubKeyHash())){
                    inputcount+=temp.getValue();
                    break;
                }
            }
        }
        if(outputcount<inputcount){
            log.info("count is not enough");
            return false;
        ...
        for (int i = 0; i < this.getInputs().length; i++) {
            ...
            if(!txInput.usesKey(prevTxOutput.getPubKeyHash()))
            {
                throw new RuntimeException("ERROR: public key is not match");
            }
            TXInput txInputCopy = txCopy.getInputs()[i];
            txInputCopy.setSignature(null);
            txInputCopy.setPubKey(prevTxOutput.getPubKeyHash());
